...
def add_game():
    ...
    conn.execute('INSERT INTO VideoGames (name, platform, year_of_release, genre, publisher, na_sales, eu_sales, jp_sales, other_sales, global_sales, critic_score, critic_count, user_score, user_count, developer, rating) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', game)
    conn.commit()
    conn.close()
...
def update_game(game_id):
    ...
    conn.execute('UPDATE VideoGames SET name = ?, platform = ?, year_of_release = ?, genre = ?, publisher = ?, na_sales = ?, eu_sales = ?, jp_sales = ?, other_sales = ?, global_sales = ?, critic_score = ?, critic_count = ?, user_score = ?, user_count = ?, developer = ?, reting = ? WHERE id = ?', game)
    conn.commit()
    conn.close()
...
def delete_game(game_id):
    conn = get_db_connection()
    conn.execute('DELETE FROM VideoGames WHERE id = ?', (game_id,))
    conn.commit()
    conn.close()
